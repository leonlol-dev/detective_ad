Shader "Hidden/Shader/Sorbel"
{
    ...
    float SorbelBasic(float topLeft, float top, float topRight,
    ...
    float SorbelScharr(float topLeft, float top, float topRight,
        float left, float right,
        float bottomLeft, float bottom, float bottomRight)
    {
    ...
    float SorbelBasic(float3 topLeft, float3 top, float3 topRight,
    ...
    float SorbelScharr(float3 topLeft, float3 top, float3 topRight,
                       float3 left, float3 right,
                       float3 bottomLeft, float3 bottom, float3 bottomRight)
    {
    ...
    float SobelSampleDepth(float2 uv, float offsetU, float offsetV)
    {
        ...
        return SorbelScharr(abs(topLeft - centre),     abs(top - centre),       abs(topRight - centre),
    ...
    {
        //if camera depth is invalid return an empty vector.
    ...
    float4 CustomPostProcess(Varyings input) : SV_Target
    {
        ...
        //This is here by default from Unity.
        ...
        // Apply sobel effect
        float3 colour = lerp(outColor, _Color, outlineIntensity * _Intensity);
